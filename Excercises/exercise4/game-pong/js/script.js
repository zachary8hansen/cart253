...
// A variable to hold the hit/contact sound we will play on bouncing
let woodSFX;
// A coin sounds when a paddle wins the round
let coin;
// A broken record plays in the background
let piano;
// The is set on a table in some saloon in Texas because let's be honest...
// it's always Texas
let table;

// preload()
//
// Loads the required sounds and images
function preload() {
  woodSFX = new Audio("assets/sounds/wood.wav");
...
//if the ball goes past the left side, right paddle height shrinks and left
//paddle height grows to level the difficulty.
  if (ball.x < 0) {
   rightPaddle.h -= 2;
   leftPaddle.h += 2;
  }

  //if the ball goes past the right side, left paddle height shrinks and right
  //paddle height grows to level the difficulty.
  if (ball.x > width) {
    leftPaddle.h -= 2;
    rightPaddle.h += 2;
